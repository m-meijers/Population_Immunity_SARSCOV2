...
countries = ['BELGIUM',
             ...
             'USA',
             'JAPAN']
country2voc2freq = defaultdict(lambda:
                               defaultdict(lambda:
...
for country in countries:
    ...
    for t in np.arange(Time.dateToCoordinate("2021-01-01"),
                       Time.dateToCoordinate("2023-04-01")):
        x_vocs = []
        for voc in all_vocs:
        ...
        x_ch1.append(x_vocs[all_vocs.index('CH.1.1')]
                     + x_vocs[all_vocs.index('CH.1')])
        x_bn1.append(x_vocs[all_vocs.index('BN.1')])
        x_bm11.append(x_vocs[all_vocs.index('BM.1.1')])

        voc_here = ['ALPHA', 'DELTA', 'BETA', 'EPSILON',
                    'IOTA', 'MU', 'OMICRON', 'GAMMA']
    ...
    country2voc2freq[country]['OMI'] = np.array(x_omi)\
        + np.array(x_ba1) + np.array(x_ba2) + np.array(x_ba45)
...
# Average frequencies
vocs = ['WT', 'DELTA', 'OMI', 'ALPHA', 'BA.1', 'BA.2', 'BA.4/5',
        'BQ.1', 'BA.4.6', 'BF.7', 'XBB', 'CH.1', 'BN.1', 'BM.1.1']
voc2freq = defaultdict(lambda: [])
for voc in vocs:
    voc2freq[voc] = np.mean([country2voc2freq[c][voc] for c in countries],
                            axis=0)

df = pd.DataFrame(voc2freq)
df.index = np.arange(Time.dateToCoordinate("2021-01-01"),
                     Time.dateToCoordinate("2023-04-01"))
