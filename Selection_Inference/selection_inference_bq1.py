...
for country in countries:
    ...
    dates_ba4 = [a for a in dates_ba4 if a < Time.dateToCoordinate("2023-03-01") and a > 44569]
    dates_ba5 = [a for a in dates_ba5 if a < Time.dateToCoordinate("2023-03-01") and a > 44569]
    dates_ba45 = sorted(list(set(dates_ba4 + dates_ba5)))
    dates_bq1 = [a for a in dates_bq1 if a < Time.dateToCoordinate("2023-03-01") and a > 44753] #before 2022-08-01
    dates_bq11 = [a for a in dates_bq11 if a < Time.dateToCoordinate("2023-03-01") and a > 44753] #before 2022-08-01
    if country == 'NY' or country == 'USA':
        dates_bq1 = [a for a in dates_bq1 if a < Time.dateToCoordinate("2023-03-01") and a > 44600] #before 2022-08-01
    ...
    while t2 + tmin < tmax:
        ...
        x_bq1_hat_t1 = np.array([x if x != 0 else 1/(N_tot1+1) for x in x_bq1_hat_t1])
        ...
        x_bq1_hat_t2 = np.array([x if x != 0 else 1/(N_tot2+1) for x in x_bq1_hat_t2])
        x_ba45_hat_t2 = np.array([x if x != 0 else 1/(N_tot2+1) for x in x_ba45_hat_t2])

        result = (np.log(x_bq1_hat_t2/x_ba45_hat_t2) - np.log( x_bq1_hat_t1 / x_ba45_hat_t1)) / dt
        ...
        np.round(s_hat_var,7), bq1_count[t1], bq1_count[t2],ba45_count[t1],ba45_count[t2],tmin,tmax,x_bq1,x_ba45])

        t1 = t1 + 7
        t2 = int(t1 + dt)


lines = pd.DataFrame(lines,columns = ['country','FLAI_time','t1','t2','s_hat','s_var','bq11_count_t1','bq11_count_t2','ba45_count_t1','ba45_count_t2','tmin','tmax','x_bq11','x_ba45'])
