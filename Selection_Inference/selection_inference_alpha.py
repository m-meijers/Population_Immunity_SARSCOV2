...
countries = ['JAPAN', 'CANADA', 'GERMANY', 'CA', 'SPAIN', 'NETHERLANDS',
             'NY', 'DENMARK', 'USA', 'SWITZERLAND', 'TX']
print("=====================1 - Alpha ============================")
...
country2max_recov = defaultdict(lambda:
                                defaultdict(lambda:
                                            defaultdict(lambda:
                                                        defaultdict())))
...
for country in countries:
    ...
    wt_count = [int(np.exp(Z[str(a)])) - int(np.exp(counts['ALPHA'][str(a)]))
                for a in t_range]
    ...
    while t2 + tmin < tmax:
        ...
        x_other_hat_t1 = np.array([x if x != 0 else 1 / (N_tot1 + 1)
                                   for x in x_other_hat_t1])
        ...
        x_other_hat_t2 = np.array([x if x != 0 else 1 / (N_tot2 + 1)
                                   for x in x_other_hat_t2])

        result = (np.log(x_alpha_hat_t2 / x_other_hat_t2)
                  - np.log(x_alpha_hat_t1 / x_other_hat_t1)) / float(dt)
        ...
        lines.append([country, FLAI_time,
                      Time.coordinateToDate(int(t1 + tmin)),
                      Time.coordinateToDate(int(t2 + tmin)),
                      np.round(s_hat, 3), np.round(s_hat_var, 7),
                      alpha_count[t1], alpha_count[t2], wt_count[t1],
                      wt_count[t2], tmin, tmax, x_wt, x_alpha, x_voc])

        t1 = t1 + 7
        t2 = t1 + dt

lines = pd.DataFrame(lines, columns=['country', 'FLAI_time', 't1', 't2',
                                     's_hat', 's_var', 'alpha_count_t1',
                                     'alpha_count_t2', 'wt_count_t1',
                                     'wt_count_t2', 'tmin', 'tmax',
                                     'x_wt', 'x_alpha', 'x_voc'])
