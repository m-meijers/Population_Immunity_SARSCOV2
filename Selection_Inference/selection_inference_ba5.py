...
for country in countries:
    ...
    dates_ba4 = [a for a in dates_ba4
                 if a < Time.dateToCoordinate("2022-10-01")
                 and a > Time.dateToCoordinate("2022-04-01")]
    dates_ba5 = [a for a in dates_ba5
                 if a < Time.dateToCoordinate("2022-10-01")
                 and a > Time.dateToCoordinate("2022-04-01")]
    dates_ba45 = sorted(list(set(dates_ba4 + dates_ba5)))
    dates_ba2 = [a for a in dates_ba2
                 if a < Time.dateToCoordinate("2022-10-01") and a > 44320]
    dates_ba22 = [a for a in dates_ba22
                  if a < Time.dateToCoordinate("2022-10-01") and a > 44320]
    dates_ba2 = sorted(list(set(dates_ba2 + dates_ba22)))
    if country == 'NY' or country == 'USA':
        dates_ba2 = [a for a in dates_ba2
                     if a < Time.dateToCoordinate("2022-10-01") and a > 44600]
    ...
    while t2 + tmin < tmax:
        ...
        result = (np.log(x_ba45_hat_t2 / x_ba2_hat_t2)
                  - np.log(x_ba45_hat_t1 / x_ba2_hat_t1)) / dt
        ...
        lines.append([country, FLAI_time,
                      ...
                      ba2_count[t1], ba2_count[t2],
                      ba45_count[t1], ba45_count[t2],
                      tmin, tmax, x_ba2, x_ba45])
        t1 = t1 + 7
        t2 = int(t1 + dt)
lines = pd.DataFrame(lines, columns=['country', 'FLAI_time',
                                     't1', 't2', 's_hat', 's_var',
                                     'ba2_count_t1', 'ba2_count_t2',
                                     'ba45_count_t1', 'ba45_count_t2',
                                     'tmin', 'tmax', 'x_ba2', 'x_ba45'])
