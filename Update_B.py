...
df_full = pd.DataFrame([],columns=['country','time','s_hat','s_var','dC_vac','dC_inf'])
for line in df_ba45.itertuples():
    ...
    new_row = pd.DataFrame([[line.country,line.time,line.s_hat,line.s_var,dC_vac,dC_inf]], columns=df_full.columns)
    df_full = pd.concat([new_row,df_full])
for line in df_bq1.itertuples():
    dC_vac = (line.C_ba45_bst - line.C_bq1_bst) + (line.C_ba45_biv - line.C_bq1_biv)
...
for time_cut_off in np.arange(Time.dateToCoordinate("2022-05-09"), Time.dateToCoordinate("2023-03-01"), 1):
    ...
    if ratio == 0:
        df_data = copy(df_full.loc[(df_full.time < time_cut_off) & (df_full.time > time_cut_off - time_window)])
    else:    
    ...
    lines.append([time_cut_off, str(Time.coordinateToDate(int(time_cut_off))), len(df_data), gamma_inf_weighted, gamma_inf_weighted / gamma_vac])
lines = pd.DataFrame(lines, columns=['time_cut_off','date_cut_off','data_points','gamma_inf_new','b_new'])
